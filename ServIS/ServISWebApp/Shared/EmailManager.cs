...
namespace ServISWebApp.Shared
{
    ...
    public class EmailManager : IDisposable
	{
		...
		private async Task<bool> ShouldSkipAsync(Thread thread)
		{
			...
			if (isSentFromApp && isAutogenerated)
			{
				 ...
				 * want to bother the admin with this message (which is meant only for the user anyway),
				 * this if was created to prevent showing this thread/message to the admin. 
