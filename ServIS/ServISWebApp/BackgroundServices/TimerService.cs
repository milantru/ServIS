...
namespace ServISWebApp.BackgroundServices
{
	public class TimerService : BackgroundService
	{
		...
		private readonly Timer everySecondTimer = new Timer(1_000);
		private readonly Timer everyTenSecondsTimer = new Timer(60_000);

		public static event Func<Task>? UpdateEverySecondEvent;

        public TimerService(IServISApi api, EmailManager emailManager, string baseUrl)
        {
			this.api = api;
		...
		{
			InitEverySecondTimer(stoppingToken);
			everySecondTimer.Start();

			InitEveryTenSecondsTimer(stoppingToken);
			everyTenSecondsTimer.Start();

			await Task.CompletedTask;
		}

		private void InitEveryTenSecondsTimer(CancellationToken stoppingToken)
		{
			// set stopping handler
			everyTenSecondsTimer.Elapsed += (object? _, ElapsedEventArgs _) =>
			{
				if (stoppingToken.IsCancellationRequested)
				{
					everyTenSecondsTimer.Stop();
					everyTenSecondsTimer.Dispose();
				}
			};

			// set work handler
			everyTenSecondsTimer.Elapsed += async (object? _, ElapsedEventArgs _) =>
			{
		...
		private void InitEverySecondTimer(CancellationToken stoppingToken)
		{
			// set stopping handler
			everySecondTimer.Elapsed += (object? _, ElapsedEventArgs _) =>
			{
				if (stoppingToken.IsCancellationRequested)
				{
			...
			// set user handlers
			everySecondTimer.Elapsed += async (object? _, ElapsedEventArgs _) =>
			{
				await (UpdateEverySecondEvent?.Invoke() ?? Task.CompletedTask);
			};
		}
	}
}
